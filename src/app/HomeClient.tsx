...
import { gameImages, storeCategoryOrder } from '@/data/cheats-meta';
...
const VISIT_KEY = 'nova_visit_count';
...
export default function HomeClient() {
  ...
  const recentlyViewed = useRecentlyViewed(3);
  ...
  const stats = useMemo(() => {
    const games = new Set(cheatCatalog.map((c) => c.category)).size;
    return {
      games,
      products: cheatCatalog.length,
      undetected: cheatCatalog.filter((c) => c.status === 'undetected').length,
    };
  }, []);
    ...
    recentlyViewed.length > 0 ? (
      <section className={styles.recentSection} aria-labelledby="recent-title">
        <div className={styles.sectionHeader}>
          <p className={styles.sectionLabel}>{t('home.continueBrowsing')}</p>
          <h2 id="recent-title" className={styles.sectionTitle}>
            {t('home.continueBrowsing')}
          </h2>
        </div>
        <div className={styles.recentGrid}>
          {recentlyViewed.map((cheat) => (
    ...
    <section className={styles.popularSection} aria-labelledby="popular-title">
      ...
      <div className={styles.popularGrid}>
        {popularCheats.map((cheat, i) => {
          ...
          const thumb = cheat.image || gameImages[cheat.category] || '/images/red-hero.webp';
        ...
        <div className={styles.heroContent}>
          <div className={styles.heroText}>
            <p className={styles.heroEyebrow}>
              {isReturning ? t('home.returningEyebrow') : t('home.heroEyebrow')}
            </p>
            <h1 id="hero-title" className={styles.heroTitle}>
              {isReturning ? (
                <>
                  {t('home.returningTitle1')}
                  <br />
                  <span>{t('home.returningTitle2')}</span>
                </>
              ) : (
                <>
                  {t('home.heroTitle1')}
            ...
            <p className={styles.heroSubtitle}>
              {isReturning ? t('home.returningSubtitle') : t('home.heroSubtitle')}
            </p>

            <div
              className={styles.heroStats}
              aria-label={language === 'en' ? 'Store stats' : 'إحصائيات المتجر'}
            >
            ...
            <div className={styles.heroActions}>
              <Link href="/store" className={styles.primaryBtn}>
                {isReturning ? t('home.returningCta') : t('home.viewAll')}
              </Link>
      ...
      <section className={styles.gamesSection} aria-labelledby="games-title">
        ...
        <div className={styles.gamesGrid}>
          {featuredCategories.map((cat, index) => {
            const count = cheatCatalog.filter((c) => c.category === cat.key).length;
            return (
              <StoreGameCard
                key={cat.key}
                label={cat.label}
